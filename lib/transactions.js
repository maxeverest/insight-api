...
TxController.prototype.list = function(req, res) {
  ...
  console.log('Transaction List/ blockHash', blockHash, 'address ', address);

  if(blockHash) {
    self.node.getBlock(blockHash, function(err, block) {
      if(err && err.message === 'Block not found.') {
		console.log('transactionlist Block not found.', err);
        return common.handleErrors(null, res);
      } else if(err) {
		console.log('transactionlist err', err);
        return common.handleErrors(err, res);
        
      }
      
      console.log('Not err GetBlock block.hash', block.hash);

      var blockInfo = self.node.services.bitcoind.getBlockIndex(block.hash);
      var txs = block.transactions;
      var totalTxs = txs.length;
      
      console.log('Not err GetBlock blockInfo', blockInfo);
      
      console.log('Not err GetBlock block.transactions', txs);
      ...
      async.mapSeries(txs, function(tx, next) {
        ...
        console.log('tx.__blockHash transactions', tx);

        tx.populateInputs(self.node.services.db, [], function(err) {
          if(err) {
			console.log('tx.populateInputs err', err);  
            //return next(err);
          }
        ...
        console.log('Not err GetBlock transformed.transactions', transformed);
